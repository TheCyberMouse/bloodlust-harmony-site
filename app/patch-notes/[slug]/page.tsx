...
export async function generateMetadata({
  ...
  if (!page || page.category !== "patch-notes")
    return { title: "Not found", robots: { index: false } };
  const description = metaDescription(
    stripHtml(page.body),
    `${page.title}: balance changes, new content and fixes for Bloodlust & Harmony.`,
  );
  const ogTitle = `${page.title} — Bloodlust & Harmony`;
  return {
    title: page.title,
    description,
    alternates: { canonical: `/patch-notes/${page.slug}` },
    openGraph: {
      title: ogTitle,
      description,
      type: "article",
...
export default async function PatchNotePage({
  ...
  const page = await getProsePage(params.slug);
  if (!page || page.category !== "patch-notes") notFound();
